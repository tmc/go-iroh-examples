// Command 02-addresses builds the value a peer is dialed with.
//
...
// service can supply the paths (05-memory-discovery).
//
...
// switching later if the network changes; 33-path-upgrade watches that happen.
//
// This example assembles an address by hand, which is what 03-direct-echo does
// on loopback and what nothing does in production. There the same value arrives
// from elsewhere: packed into a ticket (38-app-envelope-ticket) or resolved
// from DNS or pkarr (14-dns-resolve, 15-pkarr-publish-resolve).
package main
