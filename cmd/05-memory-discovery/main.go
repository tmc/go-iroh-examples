// Command 05-memory-discovery dials a peer known only by its endpoint ID.
//
// 03-direct-echo and 04-router-echo hand the client a complete address because
...
// note applies in 27-local-infra.
//
// [iroh.AddressResolver] is one interface with several implementations, and
// only this one is a map: 14-dns-resolve and 15-pkarr-publish-resolve resolve
// through n0's public deployment, 27-local-infra through a pkarr relay in the
// same process, 47-mdns-discovery over the local link.
package main
